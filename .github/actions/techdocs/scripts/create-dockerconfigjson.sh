...
create_ghcr_secrets() {
  ...
  gh_token=${3}
  local secret_args
  secret_args="lighthouse-techdocs-${repo_name}-secrets --save-config --dry-run=client --docker-server=https://ghcr.io --docker-username=${gh_user} --docker-password=${gh_token}"
